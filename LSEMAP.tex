...
\begin{document}
	...
	\begin{frame}{Related work - Multinational address parsing}
		...
		\begin{itemize}
			\item CRF based model
			\item Preprocessing
			\item '\textit{trained on over 1 billion examples in every inhabited country on Earth}' \footnote{\tiny{\url{https://medium.com/@albarrentine/statistical-nlp-on-openstreetmap-part-2-80405b988718}}}
		\end{itemize}

		\vfill

		\textbf{No previous neural network approaches for multinational address parsing}
	\end{frame}
	...
	\begin{frame}{About the Data}
		\begin{itemize}
			\item Built using the open-source data on which Libpostal's models were trained.
			\item Contain 61 countries.
			\item We used eight tags : StreetNumber, StreetName, Unit, Municipality, Province, PostalCode, Orientation, and GeneralDeliver \footnote{Libpostal used 20 tags.}.
		\end{itemize}
	\end{frame}	
	...
	\begin{frame}{Training Data and Holdout Test Set}
		20 countries are used for the multinational training with a sample size of \num{100000} per country and the rest is used as a holdout. \\\bigskip
	...
	\begin{frame}{Zero-Shot Test Set}
		41 countries are used for zero-shot transfer evaluation (never seen in training). \\\bigskip
	...
	\begin{frame}{Training Procedure}
		\begin{itemize}
			\item We trained five times\footnote{With the following seed $\{5, 10, 15, 20, 25\}$} each of our two model (\textbf{fastText} and \textbf{BPEmb}).
			\item 200 epochs with a batch size of \num{2048}. 
			...
			\item Teacher forcing \cite{6795228}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Future Work}
		\begin{itemize}
			\item Attention mechanism \cite{bahdanau2014neural}
			\item Domain adaptation techniques (e. g. DANN) (CITE)
		\end{itemize}
	\end{frame}
	\begin{frame}{Conclusion}
		\begin{itemize}
			\item Tackled the multinational address parsing problem with SOTA results.
			\item Shown that subword embeddings thelp to solve the multilingual aspect of our task.
			\item We explored the possibility of zero-shot transfer across countries and achieved interesting, but not yet optimal results.
		\end{itemize}		
	\end{frame}
